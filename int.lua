...
function int_runner:access(t,id)
	
end

function int_runner:getkey(t,id)
...
function int_runner:getvar(t,id)
	return self.vars[t][self:getkey(t,id)]
end

function int_runner:compile(c,p,l)
	self.code[c]={}
	for i=0,l-1 do
...
function int_runner:run(c)
	...
	local function _gv(p,t)
		...
		if int_genvar[t] then int_genvar[t](c,p,self)
